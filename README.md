...
/* ================= HORNS ================= */
function addHorns(g,scale){
 [-1,1].forEach(s=>{
  const h=new THREE.Mesh(new THREE.TorusGeometry(.6*scale,.15*scale,8,16),M.horn);
  h.rotation.z=Math.PI/2;
  h.position.set(s*.9,2,.2);
  g.add(h);
 });
}

/* ================= TOWERS ================= */
const towerCounts={Cube:0,Ram:0,Grazer:0,Shearer:0,Goat:0};
const towerLimits={Cube:10,Ram:4,Grazer:3,Shearer:3,Goat:2};

function cube(){const g=baseSheep(M.wool);g.damage=5;g.range=12;return g;}
function ram(){const g=baseSheep(M.wool);addHorns(g,1);g.damage=10;g.range=14;return g;}
function goat(){const g=baseSheep(M.goat);addHorns(g,3);g.damage=150;g.range=16;g.scale.set(1.2,1.2,1.2);return g;}
function grazer(){
 const g=new THREE.Group();
 ...
 g.damage=16; g.range=14; g.shoots=true; return g;
}
...
renderer.domElement.addEventListener("pointerdown",e=>{
 ...
 if(taps===1) timer=setTimeout(()=>taps=0,300);
  ...
  if(towerCounts[selected]>=towerLimits[selected]) return;
  ...
  if(!hit||wool<TOWERS[selected].cost) return;
  if(pathMeshes.some(m=>m.position.distanceTo(hit.point)<3)) return;
  wool-=TOWERS[selected].cost;
  document.getElementById("wool").textContent=wool;
  const u=TOWERS[selected].build();
  u.position.set(Math.round(hit.point.x),0,Math.round(hit.point.z));
  u.cooldown=0; towers.push(u); towerCounts[selected]++; scene.add(u);
 }
});
...
function makeEnemy(type){
 ...
 if(type==="boss"){hp=1000+bossBonus;speed=.02;woolGain=100;bossBonus+=100;}
 ...
 if(type==="boss"){
  ...
  crown.position.y=3; g.add(crown); g.scale.set(2,2,2);
 }
...
document.getElementById("startRound").onclick=()=>{
 ...
 queue=[];
 for(let i=0;i<6+round;i++) queue.push("normal");
 if(round>=5) for(let i=0;i<=round-5;i++) queue.push("fast");
 if(round>=10) for(let i=0;i<=round-10;i++) queue.push("tank");
